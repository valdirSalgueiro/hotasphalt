...
#include "../GreenJuiceTeam.Shared/game/Vector2D.h"

#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "GreenJuiceTeam.NativeActivity", __VA_ARGS__))
#define LOGW(...) ((void)__android_log_print(ANDROID_LOG_WARN, "GreenJuiceTeam.NativeActivity", __VA_ARGS__))
...
static int engine_init_display(struct engine* engine) {
	...
	* ANativeWindow buffers to match, using EGL_NATIVE_VISUAL_ID. */
	...
	if (!engine->engine)
	{
		engine->engine.reset(new Engine());
		//engine->engine->init(w, h);
		JNIEnv *jni;
		engine->app->activity->vm->AttachCurrentThread(&jni, NULL);
		engine->engine->init(800, 480);
